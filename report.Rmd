...
source("main.R") #sourcing main.R updates report.Rmd with changes made to script
```
...
`dplyr` allows easy grouping of data `tibbles`, and the `summarize` function
 ...
 **Extra:** instead of simply returning mean probe expression, re-work
 `summarize_expression()` to also return probe _variability_ by first calculating
 probe-wise variability across all samples, and adding the column to the
